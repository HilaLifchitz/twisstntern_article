...
Uses more functions from the twisstntern library and 
includes Wasserstein distance calculations (Euclidean and KL-divergence) [from twisstCompareEx.py] while 
...
L2_COLORMAP = "magma"  # Options: "mako_r", "viridis", "plasma", "inferno", "magma", "cividis", "rocket", "flare"
...
# ============================================================================
# HATCH CONFIGURATION - CONTROL EMPTY TRIANGLE APPEARANCE
# ============================================================================
# Control hatching for empty triangles in different plot types
HATCH_DATA = True      # If True, empty triangles in data/model plots are hatched. If False, plain white.
HATCH_RESIDUALS = False # If True, empty triangles in residual plots are hatched. If False, plain white.
HATCH_L2 = False        # If True, empty triangles in L2 distance plots are hatched. If False, plain white.

# Histogram color
HISTOGRAM_COLOR = "#A2C5F2" # blue
#"#ffb347"  # Orange color for residuals histogram
...
def perform_enhanced_grid_analysis(data1, data2, alpha):
    ...
    for triangle in triangles:
        ...
        prop_data = count_data / len(data1) #= f(triangle)
        prop_model = count_model / len(data2) #= g(triangle)
        
        prop_residual = prop_data - prop_model #= f(triangle) - g(triangle)
        residual_squared = prop_residual ** 2 #= (|f(triangle) - g(triangle)|)^2
        count_residual = count_data - count_model # this is NOT the residual of the L2 distance, it is the residual of the count of points in the triangle
        results_df.append({
    ...
    meaningful_triangles = (results_df['count_data'] > 0) | (results_df['count_model'] > 0) # Keeps only rows where at least one count is greater than 0
    ...
    print(f"Mean proporitional residual:            {statistics['mean_residual']:.6f}")
    print(f"Std proporitional residual:             {statistics['std_residual']:.6f}")
    print(f"Max |proporitional residual|:           {statistics['max_residual']:.6f}")
...
def plot_heatmap_data_twiss(ax, results_df, alpha, data_type='prop_data', title='Data', vmax=None, vmin=None, override_cmap=None):
    ...
    if data_type == 'count_residual':
        ...
        # Only start from 1 if max value is > 1, otherwise use actual min
        ...
        # Only start from 1 if max value is > 1, otherwise use 0
    ...
    for idx, row in results_df.iterrows():
        ...
        if empty_logic(row):
            # Determine which hatch setting to use based on data type
            if 'residual' in data_type:
                hatch_setting = HATCH_RESIDUALS
            elif data_type == 'l2':
                hatch_setting = HATCH_L2
            else:  # data or model plots
...
def plot_l2_distances_twiss(ax, results_df, alpha):
    ...
    vmin = 1 if vmax > 1 else 0  # Start from 1 only if max > 1
    ...
    for idx, row in results_df.iterrows():
        ...
        if not has_meaningful_data:
            draw_empty_triangle(ax, trianglex, triangley, hatch=HATCH_L2)
...
def create_enhanced_comparison_plot(data1, data2, results_df, statistics, alpha, output_path=None):
    ...
    min_count = 1 if max_count > 1 else 0  # Start from 1 only if max > 1
    ...
    vmin_l2 = 1 if vmax_l2 > 1 else 0  # Start from 1 only if max > 1
    plot_ternary_with_colorbar(ax4, results_df, alpha, 'l2', 'L2 Distance', vmin_l2, vmax_l2, cmap_l2, "L2 Distance")
    # Residuals histogram
    meaningful_triangles = (results_df['count_data'] > 0) | (results_df['count_model'] > 0)
...
def main():
    ...
    parser.add_argument('--no_show', action='store_true',
                       help='Skip showing plot (save only)')
    ...
    # Show plot only if not disabled
    if not args.no_show:
        print("\nDisplaying plot (close window to continue)...")
        plt.show()
    else:
        print("\nPlot saved but not displayed (use --no_show to skip display)")
